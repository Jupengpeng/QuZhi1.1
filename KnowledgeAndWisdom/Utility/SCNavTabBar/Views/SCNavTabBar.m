...
{
    _items = [@[] mutableCopy];
    _arrowImage = [[UIImage imageNamed:@"arrow.png"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
...
- (void)viewConfig
{
    ...
    if (_showArrowButton)
    {
        _arrowButton = [[UIImageView alloc] initWithFrame:CGRectMake(functionButtonX,20, ARROW_BUTTON_WIDTH, 44)];
...
//        [self viewShowShadow:_arrowButton shadowRadius:10.0f shadowOpacity:10.0f];
    ...
    _navgationTabBar = [[UIScrollView alloc] initWithFrame:CGRectMake(DOT_COORDINATE, 0, functionButtonX, 64)];
...
{
    _line = [[UIView alloc] initWithFrame:CGRectMake(2.0f, 64 - 3.0f, width - 4.0f, 2.0f)];
...
- (CGFloat)contentWidthAndAddNavTabBarItemsWithButtonsWidth:(NSArray *)widths
{
    ...
    for (NSInteger index = 0; index < [_itemTitles count]; index++)
    {
        ...
        button.frame = CGRectMake(buttonX, 20, [widths[index] floatValue], 44);
    ...
    if (pop)
    {
        ...
        [UIView animateWithDuration:0.5f animations:^{
            ...
            [UIView animateWithDuration:0.2f animations:^{
                if (!_popView)
                {
                    _popView = [[SCPopView alloc] initWithFrame:CGRectMake(0, 70, SCREEN_WIDTH, self.frame.size.height - 64)];
